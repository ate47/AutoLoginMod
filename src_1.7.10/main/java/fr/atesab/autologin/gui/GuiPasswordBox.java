...
public class GuiPasswordBox extends GuiAl {
	...
	private static void drawRightString(FontRenderer fontRenderer, String text, GuiTextField field, int color) {
		if (field.getVisible())
			fontRenderer.drawString(text, field.xPosition - fontRenderer.getStringWidth(text),
					field.yPosition + field.height / 2 - fontRenderer.FONT_HEIGHT / 2, color);
	}
	...
	public GuiPasswordBox(LoginData data, GuiScreen parent, String server, Consumer<LoginData> setter) {
		...
		this.server = server;
		this.setter = setter;
		(this.oldPassword = new GuiPasswordField(0, 0, 246, 16)).setVisible(this.checkOld = !data.emptyPassword());
		password = new GuiPasswordField(0, 0, 246, 16);
		passwordAgain = new GuiPasswordField(0, 0, 246, 16);
		fontRendererObj = Minecraft.getMinecraft().fontRenderer;
		loginPattern = new GuiTextField(fontRendererObj, 0, 0, 246, 16);
		loginPattern.setMaxStringLength(100);
		loginPattern.setText(data.getLoginPattern());
		registerPattern = new GuiTextField(fontRendererObj, 0, 0, 246, 16);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!checkPass())
			drawCenterString(fontRendererObj, I18n.format("gui.autologin.badpassword" + (checkOld ? "2" : "")),
					width / 2, y - 24, 20, 0xffcc0000);
		for (GuiTextField field : fields)
			if (field.getVisible())
				field.drawTextBox();
		drawRightString(fontRendererObj, I18n.format("gui.autologin.currentpass") + ": ", oldPassword, 0xffffffff);
		drawRightString(fontRendererObj, I18n.format("gui.autologin.password") + ": ", password, 0xffffffff);
		drawRightString(fontRendererObj, I18n.format("gui.autologin.password.again") + ": ", passwordAgain, 0xffffffff);
		drawRightString(fontRendererObj, I18n.format("gui.autologin.change.pattern.register") + ": ", registerPattern,
				0xffffffff);
		drawRightString(fontRendererObj, I18n.format("gui.autologin.change.pattern.login") + ": ", loginPattern,
				0xffffffff);
	...
	private void updateBox() {
		...
		int x = buttons[0].xPosition = width / 2 - 200;
		buttons[0].width = sX + l % buttons.length;
		x += 4 + buttons[0].width;
		buttons[0].yPosition = y;
		for (int i = 1; i < buttons.length; i++) {
			buttons[i].xPosition = x;
			buttons[i].yPosition = y;
