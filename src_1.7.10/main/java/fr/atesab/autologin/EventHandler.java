...
public class EventHandler {
	...
	public void onChatMessage(ClientChatReceivedEvent ev) {
		if (!isConnected && !mc.isSingleplayer() && mc.func_147104_D() != null) {
			if (ev.message.getUnformattedText().toLowerCase().contains("/login")) {
				if (currentLoginData.getType() == LoginDataType.SECURE_LOGIN)
					mc.displayGuiScreen(
							new GuiPasswordBox(currentLoginData.cloneEmpty(), null, mc.func_147104_D().serverIP,
									data -> mc.thePlayer.sendChatMessage(data.getLoginMessage())));
				else if (currentLoginData.emptyPassword())
					mc.displayGuiScreen(new GuiPasswordBox(currentLoginData, null, mc.func_147104_D().serverIP,
							data -> mc.thePlayer.sendChatMessage(data.getLoginMessage())));
				else
	...
	public void onConnect(ClientConnectedToServerEvent ev) {
		...
		if (!mc.isSingleplayer() && mc.func_147104_D() != null) {
			currentLoginData = ModMain.getLoginData(mc.func_147104_D().serverIP, true);
